...
The system SHALL return a paginated feed of photos that a given device UUID is watching, ordered by most recently updated first.

#### Scenario: Retrieve watcher feed page
- **WHEN** `feedForWatcher(uuid, pageNumber, batch)` is called
- **THEN** the system returns up to the configured page size of active photos watched by that UUID in descending update order

---

### Requirement: Recent feed
The system SHALL return a paginated feed of the most recently uploaded active photos, globally.
