...
1. If no active wave exists → create new wave
2. If the passed `groupingLevel` differs from the active wave's stored `groupingLevel` → create new wave
3. If the photo doesn't fit the active wave based on field matching → create new wave
4. Otherwise → add photo to active wave
...
## ADDED Requirements (replacing existing auto-group-photos spec)
