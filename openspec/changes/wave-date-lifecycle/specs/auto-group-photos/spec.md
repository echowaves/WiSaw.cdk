...
### Requirement: Auto-created wave is frozen
**Reason**: Replaced by date-derived freeze. The `frozen` boolean field no longer exists. Auto-created waves are frozen because `freezeDate` (set to newest photo date) is in the past.
**Migration**: Auto-created waves now set `splashDate` and `freezeDate` instead of `frozen = true`.
