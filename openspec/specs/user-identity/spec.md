...
The system SHALL allow a device to register a persistent anonymous identity using a globally unique nickName and a secret passphrase, tied to the device's `uuid` (a client-generated device/user identifier). All SQL queries SHALL use parameterized SQL.
...
- **THEN** a Secret record is created using parameterized SQL with the provided UUID, the nickName, and the bcrypt-hashed secret; the new record (without the secret) is returned
...
The system SHALL allow an authenticated identity to change their secret passphrase by providing their current secret. All SQL queries SHALL use parameterized SQL.
...
- **THEN** the hashed secret is replaced with the hash of `newSecret` using parameterized SQL and the updated Secret record is returned
